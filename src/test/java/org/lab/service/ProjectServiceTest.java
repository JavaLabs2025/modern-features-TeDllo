...
class ProjectServiceTest extends TestBase {
    ...
    void setUp() {
        ...
        Project project = projectService.create(managerId, "Test Project", "Description");
    ...
    void testCreateProject() {
        setCurrentUser(managerId);
        Project project = projectService.create(managerId, "New Project", "New Description");
    ...
    void testListProjectsForUser() {
        UUID testUserId = userService.register("Test User").id();
        
        setCurrentUser(testUserId);
        Project managerProject = projectService.create(testUserId, "Manager Project", "Desc");
        
        setCurrentUser(managerId);
        Project teamLeadProject = projectService.create(managerId, "TeamLead Project", "Desc");
        projectService.setTeamLead(teamLeadProject.id(), testUserId);
        
        Project developerProject = projectService.create(managerId, "Developer Project", "Desc");
        projectService.addDeveloper(developerProject.id(), testUserId);
        
        Project testerProject = projectService.create(managerId, "Tester Project", "Desc");
        projectService.addTester(testerProject.id(), testUserId);
        
        UUID otherUserId = userService.register("Other User").id();
        setCurrentUser(otherUserId);
        Project otherProject = projectService.create(otherUserId, "Other Project 1", "Desc");

        List<Project> userProjects = projectService.list(testUserId);
    ...
    void testSetTeamLeadRemovesPreviousTeamLead() {
        setCurrentUser(managerId);
        UUID previousTeamLead = userService.register("Previous TeamLead").id();
